...
Unlabeled stocks are authored in `tools/blender-vials/`. Preview or rebuild plates with `npm run vial-studio`, then `npm run publish-vial-stocks` to refresh locked masters, production assets, and `bodyBoundsPx`. Follow `tools/blender-vials/SKILL.md`.
