...
Unlabeled stocks are authored in the headless Blender / Cycles studio under `blender/`. Preview or rebuild plates with `npm run vial-studio`, then `npm run publish-vial-stocks` to refresh locked masters, production assets, and `bodyBoundsPx`. See `blender/README.md`.
