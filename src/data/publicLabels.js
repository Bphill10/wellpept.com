/**
 * Dual naming: supply sees real peptides; customer / card / invoice see
 * WellPept Renew–style public labels so the CC statement stays cosmetic.
 *
 * Customer decode: publicCode → supplyLabel (email appendix + pay page).
 * Kept free of products.js imports to avoid circular deps.
 */
...
/** Curated compound → Renew-style family + tone + code prefix. */
const COMPOUND_PUBLIC = {
  ...
  "fox04 dri": { family: "Night Seal Concentrate", tone: "Cobalt", prefix: "NS" },
...
function lookupCompound(name) {
  ...
  // Partial: longest matching key contained in compound key
  let best = null;
  let bestLen = 0;
...
/**
 * Resolve dual labels for a catalog or cart line.
 * Skincare / already-public lines keep the same name on both sides.
 */
export function resolvePublicLabels(product = {}) {
  ...
  // Already dual-labeled (e.g. re-carted order line)
  if (product.publicLabel && product.supplyLabel) {
    return {
      ...
      publicCode:
        product.publicCode ||
        buildPublicCode(
          lookupCompound(product.supplyLabel) || fallbackStyle(product.supplyLabel),
          product
        ),
    };
  }
...
function buildPublicCode(style, product) {
  ...
  const tail =
    ...
    (Number.isFinite(mg) && mg > 0
      ...
      : hashStr(product.name || style.tone).toString(36).toUpperCase().slice(0, 4));
  return `${style.prefix}-${tail}`;
}

/** Flatten order lines with public + supply fields for UI / email. */
...
/** Customer-facing line: "2× Renew Serum Kit · Cobalt 30 [RS-T30]" */
...
/** Ops / supply line: "2× T30 Tirzepatide (30mg)" */
export function formatSupplyLineLabel(line) {
  const resolved = resolvePublicLabels(line);
  const name = line.supplyLabel || resolved.supplyLabel || line.name || "item";
